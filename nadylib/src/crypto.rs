    ...
    for i in (0..source_arr.len()).step_by(2) {
        ...
        for _ in 0..32 {
            ...
            cycle[0] = cycle[0].wrapping_add(
                ((cycle[1] << 4 & 0xfffffff0).wrapping_add(key_arr[0]))
                    ^ (cycle[1].wrapping_add(total))
                    ^ ((cycle[1] >> 5 & 0x7ffffff).wrapping_add(key_arr[1])),
            );
            cycle[0] &= 0xffffffff;
            cycle[1] = cycle[1].wrapping_add(
                ((cycle[0] << 4 & 0xfffffff0).wrapping_add(key_arr[2]))
                    ^ (cycle[0].wrapping_add(total))
                    ^ ((cycle[0] >> 5 & 0x7ffffff).wrapping_add(key_arr[3])),
            );
            cycle[1] &= 0xffffffff;
        }
...
fn hex_string_to_le_array(hex: String) -> Vec<u32> {
